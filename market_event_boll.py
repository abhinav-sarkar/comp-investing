...
def find_events(ls_symbols, d_data, orders):
    ...
    for s_sym in ls_symbols:
        ...
        for i in range(1, len(ldt_timestamps)):
            ...
            #raw_input()
            if f_symboll_today < -2.0 and f_symboll_yest >= -2.0 and f_marketboll_today >= 1.4:
...
if __name__ == '__main__':
    ...
    for data in ['sp5002012']:
        ...
        with open('market-orders-boll.csv', 'w') as a:
            for t in sorted(orders, key=itemgetter(0)):
                a.write("%s,%s,%s,%s,%s,%s,\n" % (t[0].year, t[0].month, t[0].day, t[1], t[2], t[3]))

        print "Creating Study data - %s" % (data)
        filename = "EventStudy-boll.pdf"
